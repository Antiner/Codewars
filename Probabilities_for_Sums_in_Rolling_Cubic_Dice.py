...
We want to know the probability of having the sum of the results equals to 11. For that result we have only the
combination of 6 and 5. So we will have two events: {5, 6} and {6, 5}

So the probability for that result will be:

P(11, 2) = 2/(6*6) = 1/18    (The two is because we have 2 dice)
Now, we want to calculate the probability of having the sum equals to 8. The combinations for that result
will be the following: {4,4}, {3,5}, {5,3}, {2,6}, {6,2} with a total of five combinations.
...
def rolldice_sum_prob(sum_: int, dice_amount: int):
    count = 0
    for dice in itertools.product(range(1, 7), repeat=dice_amount):
        if sum(dice) == sum_:
            count += 1
...
def rolldice_sum_prob_2(sum_, dice_amount):
    import itertools
    import collections
